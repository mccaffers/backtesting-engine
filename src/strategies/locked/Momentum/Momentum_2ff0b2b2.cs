...
public class Momentum_2ff0b2b2 : BaseStrategy, IStrategy
{
    private List<OhlcObject> ohlcList = new List<OhlcObject>();

    public Momentum_2ff0b2b2(IRequestOpenTrade requestOpenTrade, IEnvironmentVariables envVariables, ITradingObjects tradeObjs, ICloseOrder closeOrder, IWebNotification webNotification) : base(requestOpenTrade, tradeObjs, envVariables, closeOrder, webNotification) { }
    ...
    public async Task Invoke(PriceObj priceObj)
    {
        ...
        ohlcList = GenericOhlc.CalculateOHLC(priceObj, priceObj.ask, TimeSpan.FromMinutes(10), ohlcList);
        ...
        if (ohlcList.Count > envVariables.randomStrategyAmountOfHHLL)
        {
            ...
            var speed = distance / (10 * envVariables.randomStrategyAmountOfHHLL);
