...
public class Momentum_438539dc_many : BaseStrategy, IStrategy
{
    private List<OhlcObject> ohlcList = new List<OhlcObject>();

    public Momentum_438539dc_many(IRequestOpenTrade requestOpenTrade, IEnvironmentVariables envVariables, ITradingObjects tradeObjs, ICloseOrder closeOrder, IWebNotification webNotification) : base(requestOpenTrade, tradeObjs, envVariables, closeOrder, webNotification) { }
    ...
    public async Task Invoke(PriceObj priceObj)
    {
        ...
        if (ohlcList.Count > envVariables.variableA)
        {
            ...
            if(tradeObjs.accountObj.pnl > 3000){
                size=4;
            }

            if(tradeObjs.accountObj.pnl > 4000){
                size=5;
            }
